    ...
    fetch(`${apiUrl}/api/blogs`)
      ...
      .then((data) => {
        ...
        const normalized = data.map((post: any) => ({
          ...
          author: post.author || (Array.isArray(post.authors) ? post.authors.join(", ") : ""),
        }))
        setPosts(normalized)
      })
      .catch(() => setPosts([]))
    ...
    if (typeof previewPost.content === "string") {
      ;(async () => {
        try {
          ...
          const rawHtml = await Promise.resolve(marked.parse(previewPost.content || ""))
            ...
            {previewPost && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-500">
                  ...
                  {previewPost.createdAt && ` | ${format(new Date(previewPost.createdAt), "PPP")}`} |{" "}
                  {previewPost.status}
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {previewPost.featured && <Badge variant="default">Featured</Badge>}
                </div>
                ...
                <div className="border rounded p-3 mt-2 min-h-[200px] bg-white">
                  {typeof previewPost.content === "string" ? (
                    previewHtml ? (
                      ...
                      <div className="text-sm text-muted-foreground animate-pulse">Rendering preview…</div>
                    )
                  ) : (
                    // editor delta JSON fallback
                    // @ts-ignore
                    <RichTextEditor content={previewPost.content} editable={false} />
                  )}
                </div>
...
function PostsGrid({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {posts.map((post, i) => (
        <PostCard key={post._id ?? i} post={post} onClick={() => onPostClick(post)} onEdit={() => onEditPost(post)} />
      ))}
    </div>
  )
}
...
const CreatePostModal = ({ open, onClose, menus, addPost, editingPost = null, onUpdate }: CreatePostModalProps) => {
  // Form States
  ...
  const [isShareUrlManuallyEdited, setIsShareUrlManuallyEdited] = useState(false)
  ...
  async function uploadToCloudinary(file: File): Promise<string> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Cloudinary upload failed: ${res.status} - ${errorDetail}`)
    }

    const data = await res.json()
    if (!data.secure_url) {
      throw new Error(data.error?.message || "Cloudinary response missing secure_url")
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-full sm:max-w-xl md:max-w-3xl lg:max-w-4xl p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">{editingPost ? "Edit Post" : "Create New Post"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-6">
            ...
            {(selectedMenu?.submenus?.length ?? 0) > 0 && (
              <div>
                ...
                <Select value={submenu} onValueChange={setSubmenu}>
                  ...
                  <SelectContent>
                    {selectedMenu?.submenus?.map((sm: { _id: string; name: string }) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center gap-3">
              ...
              <Switch checked={featured} onCheckedChange={setFeatured} className="ml-2" />
            </div>
            <div>
              <Label>Status</Label>
            ...
            <div className="flex flex-wrap items-center gap-2 border mt-2 rounded-md p-2">
              {tags.map((tag) => (
                <Badge key={tag} variant="secondary" className="flex items-center gap-1">
                  {tag}
                  <button onClick={() => handleRemoveTag(tag)} className="text-xs hover:text-red-500 ml-1">
                    ✕
                  </button>
                </Badge>
              ))}
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t">
            ...
            <Button className="bg-purple-600 text-white hover:bg-purple-700" onClick={handlePostBlog}>
