      ...
      {filteredMenus.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredMenus.map((menu) => (
            <Link key={menu._id} href={`/menu/${menu.slug}/${menu._id}`} className="group">
              <div className="h-full bg-white rounded-lg border border-gray-200 p-6 hover:shadow-lg transition-shadow">
                ...
                {menu.submenus && menu.submenus.length > 0 && (
                  <div className="space-y-2 mb-4">
                    <p className="text-xs font-semibold text-gray-500 uppercase">Submenus ({menu.submenus.length})</p>
                    <div className="flex flex-wrap gap-2">
                      {menu.submenus.slice(0, 3).map((submenu) => (
                        <span key={submenu._id} className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded">
                          {submenu.name}
                        </span>
                      ))}
